...
import {
  ...
  FiArrowRight, FiZap, FiShield, FiStar,
} from "react-icons/fi";
import {
...
import { TbBarbell, TbRocket, TbBrandWhatsapp } from "react-icons/tb";
...
const PLANS = [
  { value: "trial", label: "🎯  Free Trial Session" },
  { value: "basic", label: "Basic – 1 Month  ₹2,000" },
  { value: "standard", label: "Standard – 3 Months  ₹5,000" },
  { value: "premium", label: "Premium – 6 Months  ₹7,000" },
  { value: "elite", label: "Elite – 12 Months  ₹9,999" },
];
...
function TrustBar({ isInView }: { isInView: boolean }) {
  return (
    <motion.div
      ...
      className="flex flex-wrap sm:flex-nowrap items-center justify-center sm:divide-x divide-foreground/[0.08] border border-foreground/[0.07] rounded-2xl overflow-hidden mb-12"
    >
...
function PromoStrip({ isInView }: { isInView: boolean }) {
  return (
    <motion.div
      ...
      style={{
        border: "1px solid oklch(0.72 0.18 48 / 0.25)",
...
function InfoCard({
  ...
  return (
    <motion.div
      ...
      className="relative flex flex-col gap-3 rounded-2xl border border-foreground/[0.05] hover:border-foreground/[0.15] p-4 sm:p-5 transition-all duration-300 overflow-hidden group"
...
function WhatsAppBanner({ isInView }: { isInView: boolean }) {
  return (
    <motion.a
      ...
      className="group flex items-center gap-3 rounded-xl border border-green-500/20 bg-green-500/[0.06] hover:bg-green-500/[0.12] hover:border-green-500/40 px-4 py-3 mt-4 transition-all duration-300"
    >
...
function ContactForm({ isInView }: { isInView: boolean }) {
  ...
  const inputClass = (field: string) =>
    `w-full border rounded-xl px-4 py-3 placeholder-text-lo/40 text-sm font-mono focus:outline-none transition-all duration-200 ${errors[field]
      ...
      : "border-foreground/[0.08] focus:border-accent"
    }`;
  ...
  return (
    <motion.div
      ...
      className="relative rounded-2xl border border-foreground/[0.05] overflow-hidden"
      style={{ background: "var(--card)" }}
    >
      {/* BG */}
      <div
      ...
      {/* Form header */}
      <div className="relative border-b border-foreground/[0.05] px-6 sm:px-8 py-5">
          ...
          {submitted ? (
            ...
            >
              <motion.div
                ...
                style={{ background: "oklch(0.72 0.18 48 / 0.12)", borderColor: "var(--primary)" }}
              >
            ...
            <motion.div key="form" className="flex flex-col gap-4">
              ...
              <div>
                <label className="flex items-center gap-1.5 text-text-lo font-mono text-[9px] tracking-widest uppercase mb-2">
                  <TbBarbell size={10} /> Interested Plan
                </label>
                <select
                  name="plan" value={form.plan} onChange={handleChange}
                  className={`${inputClass("plan")} appearance-none cursor-pointer`}
                >
                  <option value="" className="bg-dark text-text-hi">Select a plan…</option>
                  {PLANS.map((p) => (
                    <option key={p.value} value={p.value} className="bg-dark text-text-hi">{p.label}</option>
                  ))}
                </select>
              </div>
              ...
              <a
                href="tel:9019342121"
                className="group flex items-center justify-center gap-2.5 border border-foreground/[0.1] hover:border-accent/40 rounded-xl py-3 text-text-lo hover:text-text-hi font-mono text-xs font-bold tracking-widest uppercase transition-all duration-300"
              >
